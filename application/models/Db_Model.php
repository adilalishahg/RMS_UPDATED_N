...
class Db_Model extends CI_Model
{
	...
	public function get_booked_flats($user_id='') {
		$this->db->distinct();

        $this->db->select('uf.flat_id');
        $this->db->from(TBL_FLAT.' uf');
        // $this->db->join('flats f', 'uf.flat_id = f.flat_id');
        $this->db->join(TBL_RENT.' r', 'uf.flat_id = r.flat_id');
		if($user_id){

			$this->db->where('uf.owner_id', $user_id);
		}
        $this->db->where('r.booked', 'yes');
        $query = $this->db->get();
        return $query->result();
    }
	...
	public function get_current_month_rent_total($user='') {
	...
	public function get_current_year_rent_total($user='') {
		// Get the current month and year
		$current_month = date('m');
		$current_year = date('Y');

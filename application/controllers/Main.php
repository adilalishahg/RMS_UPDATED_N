...
class Main extends MY_Controller
{
	...
	public function main_ajax()
	{
		...
		if ($_SESSION['role']  != '3' && $_SESSION['role']  != '5') {
			// Get the current date
			$currentDate = date('Y-m-d');
			$booked_mine = $this->Db_Model->get_booked_flats($_SESSION['user_id']);
			$booked__all_user_flats = $this->Db_Model->get_booked_flats();
			$data['total_mine_booked_flats'] = count($booked_mine); 
			...
			// $data['booked_flats'] = $this->Db_Model->get_booked_flat_by_user();
			...
			// }
			$data['total_flats_mine'] = $this->Db_Model->get_data(TBL_FLAT, $where_flats, '', '', $type = 1, $select = 'count(flat_id) as total_flats')[0]['total_flats']; 
			$data['total_flats_all'] = $this->Db_Model->get_data(TBL_FLAT, '', '', '', $type = 1, $select = 'count(flat_id) as total_flats')[0]['total_flats']; 
			$data['total_flats_all_data'] = $this->Db_Model->get_data(TBL_FLAT, '', '', '', $type = 1, $select = 'count(flat_id) as total_flats')[0]; 
			...
			$data['ratio_mine']=0;
			if ($data['total_mine_booked_flats'] && $data['total_flats_mine'] != '0') {

				$data['ratio_mine'] = ($data['total_mine_booked_flats'] / $data['total_flats_mine']) * 100;
			}
				...
				if($type=='tower_report'){
					...
					if(!empty($report)){
						foreach($report as &$rep){
							...
							$where_flat = 'SUM(monthly_rent.total_rent) AS total_amount';
	...
	public function checkout_ajax_pay()
	{
		...
		if(isset($rent_data['utility_bill'])&&$rent_data['utility_bill']!=''){
		 ...
		 if(isset($services->sweeper_id)&&!empty($services->sweeper_id)){
			...
			$sweeper_expese  = $sweeper_salary*($daysPasswed/30);
			$expense = $expense +$sweeper_expese;
			$renc_collected=$renc_collected+$sweeper_expese;
		 ...
		 if(isset($services->watchman_id)&&!empty($services->watchman_id)){
			...
			$watchman_expese  = $watchman_salary*($daysPasswed/30);
			$expense = $expense +$watchman_expese;
			$renc_collected=$renc_collected+$watchman_expese; 
		}  
		$data=array('booked' => 'no','paid' => 'yes','expense'=>round($expense),'total_rent'=>round($renc_collected));
		$data1['status'] = '1';
		$user_id = $this->Db_Model->update_data(TBL_RENT, $data, $where2);
		$user_id = $this->Db_Model->update_data(TBL_FLAT, $data1, $where);
		$where_tw = array('tenant_id'=>$rent_data['tenant_id'],'flat_id'=>$rent_data['flat_id']);

function load_reports(data='') {
	...
	$("#report_search").on("click", function () {
		 ...
		 loadReportHtml(report, start_date, end_date, name)
	}) 
} 
function loadReportHtml(report='', start_date='', end_date='', name=''){
	console.log(report)
	localStorage.setItem('report_type',report)
	localStorage.setItem('start_date',start_date)
	localStorage.setItem('end_date',end_date)
	localStorage.setItem('name',name)
	let tableHeadFoot='';
	let tableHeader=`<tr id='tbl_head'><th>#</th>`;
	let tableFooter=`<tr><th>#</th>` 
	...
	$(".loader").hide();
		...
		if(data1.report&&data1.report.length>0){
			...
			$('table thead').html('');
			// if($('table thead').html()==''){
				console.log(tableHeader+tableHeadFoot) 

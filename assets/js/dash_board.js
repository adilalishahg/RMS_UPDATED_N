...
function load_dashboard(data) {
	...
	const booked_ratio =  data.total_mine_booked_flats||"0" + `/` + data.total_flats_mine||"0" + " ";
	console.log(booked_ratio)
	var res_dashboard = `<div class="row">`;
	res_dashboard += card(
		"primary",
		"EARNINGS (MONTHLY)",
		"$" + data.get_current_month_rent_total_mine?data.get_current_month_rent_total_mine:"0",
		"fa-calendar"
	);
	res_dashboard += card(
		"primary",
		"EARNINGS (Annual)",
		"$" + data.get_current_year_rent_total_mine||"0",
		"fa-calendar"
	);
	res_dashboard += card(
		"info",
...
const  doughnut=()=>{
	...
	html += 					'<div class="card shadow mb-4 p-4">'
	html += 						'<!-- Card Header - Dropdown -->'
	html += 						'<h5 class="m-0 font-weight-bold text-primary">Earnings/Expense</h5>'
	html += 					'</div>'
	html += 					'<!-- Card Body -->'
	html += 					'<div class="card-body">'
	html += 						'<div class="chart-pie pt-4">'
